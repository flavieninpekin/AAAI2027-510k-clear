...
STATIC (revealed): partner type visible â†?agent specializes â†?Îº â‰?0.5
DYNAMIC (hidden): partner switches hidden â†?gradient death â†?Îº â†?0
"""
...
if __name__ == '__main__':
    ...
    for mode in MODES:
        ...
        for seed in SEEDS:
            ...
            print(f'DQN {mode} seed{seed}:  Îº={k:.4f}  r_chef={rc:.1f}  r_waiter={rw:.1f}')
            env.close()
    ...
    for mode in MODES:
        ...
        print(f'{mode}:')
        print(f'  Îº:   mean={np.mean(vals):.4f}  std={np.std(vals):.4f}')
