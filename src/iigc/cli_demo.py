"""
510K CLI Demo �?人机对战，验证规则是否正确实�?"""
import sys, re, os
import io
sys.stdout = io.TextIOWrapper(sys.stdout.buffer, encoding='utf-8')

from iigc.env.env_510k import FiveTenKEnv
from iigc.env.card import Card, Rank, Suit
from iigc.env.patterns import PatternType
...
PATTERN_NAMES = {
    ...
    PatternType.THREE_PAIR: '三带一�?, PatternType.STRAIGHT: '顺子',
...
def cli_demo(mode='single'):
    ...
    deal = log[0]
    np_str = '18�? if game.num_players == 3 else '13�?
    print('\n' + '=' * 60)
    ...
    while True:
        ...
        while last_log_idx < len(log):
            ...
            if entry['action'] == 'play':
                ...
                print(f'  P{entry["player"]}  �?)

            elif entry['action'] == 'trick_end':
                winner = entry['winner']
                sc = entry.get('score', 0)
                if sc > 0:
                    print(f'  �?P{winner} 赢得此轮，得 {sc} �?)
                else:
        ...
        print(f'你的手牌 ({len(game.players[human_id].hand)} �?  [510K�? {score}]')
        ...
        if finished:
            print(f'  已出�? P{", P".join(str(p) for p in finished)}')
    ...
    for i in range(game.num_players):
        status = '�?出完' if game.players[i].finished else f'{len(game.players[i].hand)}�?
        r = rewards.get(i, 0)
        s = game.player_510k_scores[i]
        sign = '+' if r >= 0 else ''
        print(f'  P{i}: {status}  510K�? {s}  总得�? {sign}{r:.0f}')
