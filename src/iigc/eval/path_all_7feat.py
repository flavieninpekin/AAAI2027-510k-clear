...
for mode, seeds in JOBS.items():
    for seed in seeds:
        ...
        if len(mus) >= 2:
            ...
            print(f'  â†?PATH={path_len:.4f} ENDPT={endpt:.4f} CURV={curv:.1f}x', flush=True)
...
for mode in ['single', 'static', 'dynamic']:
    ...
    if vals:
        score_spreads = [v['end'][5] for v in vals]  # Ï†â‚?index 5
        sup_gaps = [v['end'][6] for v in vals]  # Ï†â‚?index 6
